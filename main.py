...
for ds in DATASETS:
    ...
    print(f"Dados carregados: {len(dados)} amostras, {dados.shape[1]} features")
    ...
    # K-means
    print(f"\n--- Executando K-means para k de {ds['kmin']} a {ds['kmax']} ---")
    for k in range(ds["kmin"], ds["kmax"]+1):
        ...
        plot_clusters(dados, rotulos_pred, ids, graph_path, titulo=f"{ds['nome']} K-means k={k}")
    ...
    for modo in ['single', 'complete']:
        ...
        for k in range(ds["kmin"], ds["kmax"]+1):
            ...
            graph_path = os.path.join(GRAPH_DIR, f"{ds['nome']}_linkage_{modo}_k{k}.png")
            plot_clusters(dados, rotulos_pred, ids, graph_path, titulo=f"{ds['nome']} {modo}-linkage k={k}")
            print(f"Gráfico salvo: {graph_path}")
